...
function PatientPage() {
  ...
  const handleGenerateReport = () => {
    ...
    doc.text(`Total Patients: ${stats.total}`, pageWidth - margin, yPosition, { align: "right" });
    yPosition += 30;

    // Summary Boxes
    const summaryStats = [
      { title: 'Pregnant Mothers', value: stats.types['Pregnant Mother'] || 0, color: [106, 90, 205] },
      { title: 'Babies', value: stats.types['Baby'] || 0, color: [129, 199, 132] },
      { title: 'Children', value: stats.types['Child'] || 0, color: [76, 175, 80] },
    ];

    summaryStats.forEach((stat, index) => {
      const x = margin + (index * 180);
      if (x < pageWidth - 100) {
        doc.setFillColor(stat.color[0], stat.color[1], stat.color[2], 20);
        doc.roundedRect(x, yPosition, 160, 60, 3, 3, 'F');
        doc.setTextColor(stat.color[0], stat.color[1], stat.color[2]);
        doc.setFontSize(14);
        doc.text(stat.title, x + 10, yPosition + 20);
        doc.setFontSize(18);
        doc.text(stat.value.toString(), x + 10, yPosition + 40);
      }
    });
    yPosition += 100;
    ...
    autoTable(doc, {
      startY: yPosition,
      head: [['Name', 'Patient Type', 'Registration Date', 'Contact']],
      body: filteredPatients.map(patient => [
        patient.details?.name || 'N/A',
        patient.patientType,
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      doc.text(`Generated by CareTrack Pro`, margin, doc.internal.pageSize.getHeight() - 20);
    }
  ...
  <div className="flex flex-col md:flex-row items-start md:items-center justify-between">
    ...
    <div className="flex gap-3 mt-4 md:mt-0">
      <button
        ...
        className="flex items-center px-4 py-2.5 bg-[#4B0082] hover:bg-[#6A5ACD] text-white text-sm font-medium rounded-lg transition-colors shadow-sm"
      >
